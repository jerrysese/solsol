...
| 13 | 작업 디렉토리(또는 작업 참고 디렉토리)가 지정되면 **코드 작성 전 가장 먼저** `capture/{디렉토리}/full_screen.jpg`를 Read로 열어 전체 구조·텍스트·수치를 추출한다. 디렉토리 표기 방식(「작업 디렉토리」「참고 디렉토리」등)에 관계없이 동일하게 적용한다. 이 단계를 건너뛰고 텍스트를 추측·임의 작성하는 것은 퍼블리싱 오류다. |
